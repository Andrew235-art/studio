...
/**
 * @fileOverview AI-enhanced inquiry assistant for Stamerck. It answers user queries and suggests additional services.
 *
...
const AIInquiryAssistantInputSchema = z.object({
  inquiry: z.string().describe('The user inquiry about Stamerck services.'),
});
...
const prompt = ai.definePrompt({
  ...
  prompt: `You are an AI assistant for Stamerck, a non-emergency medical transportation company.  A user has submitted the following inquiry: {{{inquiry}}}.\n\nRespond to the inquiry directly and suggest additional relevant services that Stamerck offers, so that the user can quickly get the information they need and understand all available options.  Focus on services that may not be immediately obvious based on the inquiry, suggesting services the user may not have considered.\n\nFormat your response as follows:\n\nResponse: [Your direct answer to the user's inquiry]\nSuggested Services: [A comma separated list of suggested services]`,
});
